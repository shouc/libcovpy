...
struct cov_context* new_context(int id){
    struct cov_context* x = malloc(sizeof(struct cov_context));
    x->id = id;
    return x;
}

struct edge_set* new_edge_set(){
    struct edge_set* x = malloc(sizeof(struct edge_set));
    return x;
}

void free_context(struct cov_context* x){
    free(x);
}

void free_edge_set(struct edge_set* x){
    free(x);
}
...
uint64_t get_count(struct edge_set* x){
    return x->count;
}

void* get_arr(struct edge_set* x){
    return x->edges;
}

uint64_t found_edge(struct cov_context* x){
    return x->found_edges;
}
